...
import com.sluckframework.domain.aggregate.EventRegistrationCallback;
...
public class AggregateEventContainer implements Serializable {
	...
	public AggregateEvent addEvent(AggregateEvent aggregateEvent) {
		if (aggregateEvent.getAggregateIdentifier() == null) {
			aggregateEvent = new GenericAggregateEvent(aggregateEvent, aggregateIdentifier);
		}
	...
	public void addEventRegistrationCallback(
		...
		for (int i = 0; i < events.size(); i++) {
			events.set(i,
					eventRegistrationCallback.onRegisteredEvent(events.get(i)));
		}
	}
